...
public class ViewFinderView extends View implements IViewFinder {
    ...
    private static volatile Rect mFramingRect;
    ...
    private final int mDefaultBorderLineColor = getResources().getColor(R.color.viewfinder_border_line);
    ...
    private void init() {
        ...
        mBorderLinePaint.setColor(mDefaultBorderLineColor);
        mBorderLinePaint.setStyle(Paint.Style.STROKE);
        mBorderLinePaint.setStrokeWidth(mDefaultBorderStrokeWidth);
    ...
    public void setBorderLineColor(int borderLineColor) {
        mBorderLinePaint.setColor(borderLineColor);
    }
    ...
    @Override
    public void setupViewFinder(final Rect rect) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                updateFramingRect(rect);
                invalidate();
            }
        }, 300L);
    }
...
//        drawViewFinderBorderLine(canvas);
    ...
    private void drawViewFinderBorderLine(Canvas canvas) {
        Rect framingRect = getFramingRect();

        // Top-left corner
        Path path = new Path();
        path.moveTo(framingRect.left + mBorderLineLength, framingRect.top);
        ...
        path.lineTo(framingRect.right, framingRect.bottom - mBorderLineLength);
        path.moveTo(framingRect.right - mBorderLineLength, framingRect.bottom);
        ...
        path.lineTo(framingRect.left, framingRect.top + mBorderLineLength);
    ...
    protected void onSizeChanged(int xNew, int yNew, int xOld, int yOld) {
        updateFramingRect();
        Log.d("onSizeChanged", "updateFramingRect()");
    }
    ...
    public synchronized void updateFramingRect(Rect rect) {
        mFramingRect = new Rect(rect.left + (int) mBorderPaint.getStrokeWidth() / 2, rect.top + (int) mBorderPaint.getStrokeWidth() / 2, rect.right - (int) mBorderPaint.getStrokeWidth() / 2, rect.bottom - (int) mBorderPaint.getStrokeWidth() / 2);
    }
}
